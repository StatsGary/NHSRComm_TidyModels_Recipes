...
The next section shows how to use the stacks package to ensemble our decision tree, logistic regression and random forest together.
...
## Collecting the meta stacked model predictions

The next step we will collect the predictions from the models:

```{r collect_model_preds}
lr_stack <- lr_fit %>% 
  collect_predictions() %>% 
  select(id, .row, stranded_class, .pred_class)

rf_stack <- rf_fit %>% 
  collect_predictions() %>% 
  select(id, .row, stranded_class, .pred_class)

xgboost_stack <- xgboost_fit %>% 
  collect_predictions() %>% 
  select(id, .row, stranded_class, .pred_class)
```
...
We will join these together in a data frame:

```{r create_stacked_ensemble}
stack_df <- lr_stack %>% 
  left_join(rf_stack) %>% 
  left_join(xgboost_stack)
...
The next step is to create the meta model for making predictions against:
...
meta_stack_model <-
  ...
  set_mode("classification") %>% 
  set_engine("kernlab") %>% 
  fit(stranded_class ~ ., data = stack_df)

# Tidy the outputs

```

## Create final data frame with meta model 

```{r meta_stacked_preds}
prob_preds_meta <- predict(meta_stack_model, test_data, type="prob")
clas_preds_meta <- predict(meta_stack_model, test_data)
